...
\caption{Per-layer metric heatmap. Each column: one of five metrics (min-max normalized). Red dashed: CR-composite layer; green dashed: threshold-based layer. CR-composite selections cluster near input layers; threshold-based metrics peak at deep layers.}
...
\centering
\small
\caption{Multi-metric layer ensemble vs.\ best single metric and text baseline. All Ens--Text differences significant ($p \leq 0.002$); no Ens--Best difference significant ($p > 0.27$).}
\label{tab:ensemble}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lcccc}
\toprule
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig2_temporal_improvement.pdf}
...
\bottomrule
\end{tabular}
\caption{HS vs.\ text at threshold-based layers (logistic regression, 60/20/20 split, 10{,}000 bootstrap). Enc.: text encoder (384d = MiniLM, 1024d = BGE-large). $\Delta$: HS$-$text difference (pp); ``--'' = not computed (different calibration points). HB: \checkmark{} = passes Holm--Bonferroni ($\alpha{=}0.05$). $^\dagger$\,Sample-limited ($n \leq 20$).}
\label{tab:hs_vs_text}
\end{table*}

\subsection{32B Scale}\label{sec:32b-analysis}

Results at 32B are suggestive due to small test sets ($n{=}19$--$20$; minimum detectable effect (MDE) 8.6--9.5pp at 80\% power). R1-32B shows early detection ($W{=}1$, +14.2pp over position) but the residualized gap (+1.9pp) is not significant; QwQ-32B achieves significance at all window sizes but with negative lead time and a competitive position baseline. Neither provides confirmatory evidence; replication with $n \geq 50$ traces is needed (\cref{app:32b_full}).\looseness=-1

\subsection{Temporal Context}\label{sec:temporal}

Temporal windowing ($W{>}1$) improves both modalities (\cref{fig:window_ablation}; cross-model and text-probe ablations in \cref{tab:window_full,app:text_window}): at 7--8B, the modality advantage (+20.1 to +21.9pp) exceeds temporal improvement (+13pp). GRU models achieve comparable accuracy but higher FPR (\cref{tab:gru_multi_seeds,app:temporal_details}); an adaptive two-stage strategy offers marginal gains (\cref{app:two_stage}). CP-aligned probe trajectories confirm sharper HS transitions at the commitment point (\cref{fig:trajectory}). An oracle analysis (\cref{sec:oracle}) places the early detection ceiling at 50--94\% across models, framing the gap that learned probes must close.\looseness=-1
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.78\textwidth]{figures/fig_trajectory.pdf}
